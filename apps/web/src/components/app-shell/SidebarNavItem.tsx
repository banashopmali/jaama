...
export const SidebarNavItem: React.FC<SidebarNavItemProps> = ({ item }) => {
  const { isCollapsed, activePath, setActivePath } = useAppShell();
  ...
  return (
    ...
    >
      ...
      {!isCollapsed && item.badge && (
        <span className="ml-auto px-1.5 py-0.5 text-[10px] font-bold rounded-md bg-surface-brand-subtle text-content-brand border border-border-brand-subtle">
          {item.badge}
        </span>
      )}
