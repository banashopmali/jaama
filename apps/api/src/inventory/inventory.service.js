...
let InventoryService = class InventoryService {
    async recordAdjustment(userContext, dto, prismaClient = database_1.prisma) {
        const organizationId = userContext.organizationId;
        const { productId, movementType, quantityDelta, reference } = dto;
        if (!productId) {
            throw new common_1.BadRequestException("L'identifiant du produit est obligatoire.");
        }
        if (!movementType) {
            throw new common_1.BadRequestException("Le type de mouvement de stock est obligatoire.");
        }
        if (quantityDelta === undefined || quantityDelta === 0) {
            throw new common_1.BadRequestException("La quantité de mouvement doit être différente de zéro.");
        }
        return prismaClient.$transaction(async (tx) => {
            ...
            if (!product) {
                throw new common_1.NotFoundException("Produit introuvable ou n'appartient pas à votre organisation.");
            }
            ...
            if (newAvailable < 0) {
                throw new common_1.BadRequestException(`Stock insuffisant. Quantité disponible actuelle : ${balance.availableQuantity}, ajustement demandé : ${actualDelta}. Le stock ne peut pas être négatif.`);
            }
      ...
      `;
            if (updatedCount === 0) {
                throw new common_1.BadRequestException("Échec de l'ajustement du stock (concurrence ou stock négatif).");
            }
    ...
    async listInventory(userContext, query = {}, prismaClient = database_1.prisma) {
        const organizationId = userContext.organizationId;
        const page = Math.max(1, query.page || 1);
        const limit = Math.max(1, Math.min(100, query.limit || 20));
        const skip = (page - 1) * limit;
        const where = { organizationId, status: { in: ["active", "inactive"] } };
    ...
    async getStockMovements(userContext, productId, limit = 50, prismaClient = database_1.prisma) {
        const organizationId = userContext.organizationId;
