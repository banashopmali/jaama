...
let ProductsService = class ProductsService {
    async createProduct(userContext, dto, prismaClient = database_1.prisma) {
        const organizationId = userContext.organizationId;
        const skuClean = dto.sku.trim().toUpperCase();
        if (!dto.name || dto.name.trim().length === 0) {
            throw new common_1.BadRequestException("Le nom du produit est obligatoire.");
        }
        if (!skuClean) {
            throw new common_1.BadRequestException("Le SKU du produit est obligatoire.");
        }
        if (dto.unitPriceMinor === undefined || dto.unitPriceMinor < 0) {
            throw new common_1.BadRequestException("Le prix unitaire doit être un entier positif ou nul.");
        }
        return prismaClient.$transaction(async (tx) => {
            ...
            if (existing) {
                throw new common_1.BadRequestException("Un produit avec ce SKU existe déjà dans votre organisation.");
            }
            // 2. Create Product
    ...
    async updateProduct(userContext, productId, dto, prismaClient = database_1.prisma) {
        const organizationId = userContext.organizationId;
        return prismaClient.$transaction(async (tx) => {
            const existing = await tx.product.findUnique({
                where: {
            ...
            if (dto.sku && dto.sku.trim().toUpperCase() !== existing.sku) {
                ...
                if (skuCheck) {
                    throw new common_1.BadRequestException("Un autre produit utilise déjà ce SKU dans votre organisation.");
                }
            }
    ...
    async getProduct(userContext, productId, prismaClient = database_1.prisma) {
        const organizationId = userContext.organizationId;
    ...
    async listProducts(userContext, query = {}, prismaClient = database_1.prisma) {
        const organizationId = userContext.organizationId;
    ...
    async archiveProduct(userContext, productId, prismaClient = database_1.prisma) {
